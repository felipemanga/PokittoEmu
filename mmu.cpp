...
template< u8* buffer, u32 base, u32 size, typename retType > retType readBuffer( u32 addr ){
    if( addr < base ){
...
template< typename valType > void writeROR( u32 addr, valType ){
    // throw?
    std::cout << " Write to read-only region: " << std::hex << addr << std::endl;
}

template< u8* buffer, u32 base, u32 size, typename valType > void writeBuffer( u32 addr, valType value ){
    if( addr < base ){
...
MemoryBank sysconBank = {
    &readRegister<  SYS::layout, 0x40048000, sizeof(SYS::layout), u32 >,
    &readRegister<  SYS::layout, 0x40048000, sizeof(SYS::layout), u16 >,
    &readRegister<  SYS::layout, 0x40048000, sizeof(SYS::layout), u8  >,
    &writeRegister< SYS::layout, 0x40048000, sizeof(SYS::layout), u32 >,
    &writeRegister< SYS::layout, 0x40048000, sizeof(SYS::layout), u16 >,
    &writeRegister< SYS::layout, 0x40048000, sizeof(SYS::layout), u8  >
};
